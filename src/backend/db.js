...
function FirebaseDatabase(firebase, deviceKey) {
  if (!(this instanceof FirebaseDatabase))
    return new FirebaseDatabase(firebase, deviceKey);
  ...
  self.getPreviousStatesInternal = function getPreviousStatesInternal(
    callback
  ) {
  ...
  self.processSubscriptionsInternal = function processSubscriptionsInternal(
    callback
  ) {
    subscriptionRef.once('value', (snapshot) =>
      snapshot.forEach((child) => callback(child.val()))
    );
  };
  ...
  const truncateData = function truncateData() {
    ...
    const toDeleteRef = stateRef
      .orderByChild('timestamp')
      .endAt(Date.now() - period);
...
FirebaseDatabase.prototype.getPreviousStates = function getPreviousStates(
  callback
) {
...
FirebaseDatabase.prototype.processSubscriptions = function processSubscriptions(
  callback
) {
